...
function setup() {
  ...
  // 設定偵測參數，啟用 refineLandmarks 以獲得更精準的瞳孔點
  const options = {
    maxFaces: 1,
...
function draw() {
  ...
  // 檢查圖片是否載入成功
  if (!maskImage || maskImage.width <= 1) {
    return;
  }
  ...
  offscreen.push();
  offscreen.translate(eyeMidX, eyeMidY);
  offscreen.rotate(rawAngle);
  offscreen.image(maskImage, -offsetX, -offsetY, drawW, drawH);
  offscreen.pop();
